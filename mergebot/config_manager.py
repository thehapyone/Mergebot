    ...
    if platform_type == "gitlab":
        ...
        try:
            ...
            if repo_config is not None:
                ...
                logger.info(
                    "Successfully loaded and validated .mergebot.yml from repo."
                )
            else:
                logger.warning(
                ...
                if existing_pr_url:
                    ...
                    logger.error("Onboarding required. Please merge the existing PR to enable Mergebot.")
                    sys.exit(1)
                else:
        ...
        logger.error(
            "GitHub support for repo config onboarding is not yet implemented."
        )
        sys.exit(1)
    else:
